...
- **VI-B.0b**: lado das atribuições: split de `δ : ↥B → Polymer` com `δ r = η` em (valores do tail); igualdade de pesos por aresta (`edgeIndicatorOn δ` vs `hardCoreEdgeIndicator (rootedTuple γ₀ ·)` via `rootedTuple_succ` + valores locais; atividade `rootedActivityOn` vs produto do tail); o **sum_bij por bloco**: `Σ_{d : BlockDatum N B} blockDatumWeight = markedBlockContribution ρ γ₀ B` — reindexar `BlockDatum ≃ Σ r, Σ η, (tail × árvore-ambiente)` e por (r,η) usar a correspondência 0a + `fixedRootBlockSum`.
