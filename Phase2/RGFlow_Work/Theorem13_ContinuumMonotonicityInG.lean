...
- Pattern: perfectly linear, slope -0.3 GeV/unit
...
/-- **Theorem 7 (Lattice Monotonicity in g):**
    Δ(g₁, a) > Δ(g₂, a) when g₁ < g₂ and a ∈ (0, 0.2].
...
axiom continuum_gap_quantitative_separation
    ...
    (hg₂ : 0.5 ≤ g₂ ∧ g₂ ≤ 1.18)
    (h_order : g₁ < g₂) :
    0.2 * (g₂ - g₁) ≤ Delta0 g₁ - Delta0 g₂

/-! ## Auxiliary Lemmas -/

/-- The set (0, 0.2] is in the right neighborhood filter at 0. -/
...
/-- **Step 1: Lattice gap difference is eventually positive.**
    d(a) := Δ(g₁, a) - Δ(g₂, a) > 0 for all a ∈ (0, 0.2],
...
theorem continuum_monotonic_in_g
  ...
  -- We have: Δ₀(g₁) - Δ₀(g₂) ≥ 0.2 · (g₂ - g₁) > 0
  have h_sep := continuum_gap_quantitative_separation g₁ g₂ hg₁ hg₂ h_order
  have h_pos : (0 : ℝ) < 0.2 * (g₂ - g₁) := by
    apply mul_pos (by norm_num : (0 : ℝ) < 0.2)
    linarith
  linarith
...
/-- **Corollary 13b: Quantitative gap bound.**
    The gap between Δ₀(g₁) and Δ₀(g₂) is at least 0.2·|g₁-g₂|. -/
...
/-- **Corollary 13d: The complete continuum picture.**
    ...
    - Δ₀(g₁) - Δ₀(g₂) ≥ 0.2·(g₂-g₁) (quantitative gap) -/
...
|---------|-------------|-------------|------------------------------|----------------|
| Thm 11  | Positivity  | Δ₀ ≥ 0.5  | ge_of_tendsto               | 190% margin    |
...
- **Positive:** bounded below by 0.5 GeV
- **Lipschitz:** with constant ≤ 2.0 (observed: 0.3)
- **Strictly decreasing:** with rate ≥ 0.2 GeV/unit
