...
where:
- Δ_max = Δ₀(0.50) = 1.655 GeV (maximum, at weakest coupling)
...
|---------|---------------|-----------------------|
| Thm 11  | Positivity    | Provides floor (≥0.50)|
...
1. **Monotonicity (Thm 13):** Δ₀ is strictly decreasing on [0.5, 1.18]
   → Maximum at left endpoint (g = 0.50)
   → Minimum at right endpoint (g = 1.18)
2. **Upper bound:** For any g ∈ [0.5, 1.18], g ≥ 0.50,
   so by monotonicity Δ₀(g) ≤ Δ₀(0.50) = 1.655
...
/-- **Theorem 15: Universal Physical Bound (Grand Synthesis)**
    ...
    **Proof:**
    ...
    2. Upper bound: g ≥ 0.50 → Δ₀(g) ≤ Δ₀(0.50) = 1.655
...
/-- **Corollary 15a: Mass gap is strictly positive (strengthened).**
    Δ₀(g) ≥ 1.452 > 0.50 > 0 for all g ∈ [0.5, 1.18].
    This is much stronger than Theorem 11's bound of 0.50 GeV. -/
...
theorem mass_gap_amplitude :
    Delta0 0.50 - Delta0 1.18 = (0.203 : ℝ) := by
...
/-- **Corollary 15d: Extremes are achieved at endpoints.**
    The maximum is at g = 0.50, the minimum at g = 1.18. -/
theorem extremes_at_endpoints :
    (∀ g : ℝ, (0.5 ≤ g ∧ g ≤ 1.18) → Delta0 g ≤ Delta0 0.50) ∧
...
4. ✅ Mass Gap Lower Bound (Δ ≥ 0.50 GeV)
...
11. ✅ Continuum Mass Gap Lower Bound (Δ₀ ≥ 0.50 GeV) 💎
...
|---|------------------|-------------|------------------------|
| 11| Positivity       | Δ₀ ≥ 0.50   | ge_of_tendsto         |
